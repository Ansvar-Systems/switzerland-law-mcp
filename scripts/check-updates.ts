#!/usr/bin/env tsx
/**
 * Swiss Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.fedlex.admin.ch';
const PORTAL_NAME = 'Fedlex (Swiss Federal Chancellery)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/swiss-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Swiss Law MCP — Data Freshness Check');
