#!/usr/bin/env tsx
/**
 * Database builder for Switzerland Law MCP server.
 *
 * Builds the SQLite database from seed JSON files in data/seed/.
 * Follows the UK Law MCP reference pattern.
 *
...
function buildDatabase(): void {
  console.log('Building Switzerland Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Swiss statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'CH');
    insertMeta.run('source', 'fedlex.admin.ch');
    insertMeta.run('licence', 'Open Government Data (OGD)');
  });
  writeMeta();
