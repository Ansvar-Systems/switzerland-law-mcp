#!/usr/bin/env tsx
/**
 * Drift detection for Switzerland Law MCP.
 *
 * Checks if upstream Fedlex content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Switzerland Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Switzerland-Law-MCP/1.0 drift-detect' },
      });
