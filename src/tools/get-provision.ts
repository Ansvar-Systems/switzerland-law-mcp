/**
 * get_provision — Retrieve specific provision(s) from a Swiss statute.
 */
...
export async function getProvision(
  ...
  if (ref) {
    ...
    // Try with "art" prefix (e.g., "1" -> "art1")
    if (!provision) {
    ...
    if (provision) {
      return {
        results: [{
          ...
          article_number: String(provision.provision_ref).replace(/^art/, ''),
  ...
  return {
    results: provisions.map(p => ({
      ...
      article_number: String(p.provision_ref).replace(/^art/, ''),
      url: docRow.url ?? undefined,
    })),
    _metadata: generateResponseMetadata(db),
  };
