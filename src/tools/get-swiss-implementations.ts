/**
 * get_swiss_implementations — Find Swiss statutes implementing a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetSwissImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface SwissImplementationResult {
  document_id: string;
...
export async function getSwissImplementations(
  db: InstanceType<typeof Database>,
  input: GetSwissImplementationsInput,
): Promise<ToolResponse<SwissImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as SwissImplementationResult[];
