/**
 * search_eu_implementations — Search EU directives/regulations with Swiss implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_swiss_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as swiss_statute_count
  ...
  if (input.has_swiss_implementation) {
    sql += ' HAVING swiss_statute_count > 0';
  }
