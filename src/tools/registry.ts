/**
 * Tool registry for Switzerland Law MCP Server.
...
import { getSwissImplementations, type GetSwissImplementationsInput } from './get-swiss-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Fedlex (Swiss Federal Chancellery). ' +
    ...
    'Call this FIRST when you need to understand what Swiss legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Swiss statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
      ...
      'Results may be in German, French, Italian, or English depending on available translations. ' +
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description:
            'Search query in German, French, Italian, or English. Supports FTS5 syntax: ' +
            '"Datenschutz" for exact term, "protection données" for phrase, term* for prefix.',
        },
        document_id: {
          type: 'string',
    ...
    inputSchema: {
      ...
      properties: {
        document_id: {
          type: 'string',
          description:
            'Statute identifier: SR number (e.g., "SR 235.1"), abbreviation (e.g., "DSG", "StGB"), ' +
            'full title (e.g., "Federal Act on Data Protection"), or internal document ID.',
        },
        section: {
          type: 'string',
          description: 'Section/article number (e.g., "1", "143bis"). Omit to get all provisions.',
        },
        provision_ref: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Validate a Swiss legal citation against the database — zero-hallucination check. ' +
      ...
      'Supports formats: "Art. 1 DSG", "SR 235.1 Art. 1", "Art. 143bis StGB".',
    inputSchema: {
      type: 'object',
      properties: {
        citation: {
          type: 'string',
          description: 'Citation string to validate. Examples: "Art. 1 DSG", "SR 235.1 Art. 1".',
        },
      },
      required: ['citation'],
    },
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Swiss statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Switzerland?" ' +
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'Legal question or topic to research (e.g., "Datenschutz", "data protection").',
        },
        document_id: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Format a Swiss legal citation per standard conventions. ' +
      'Three formats: "full" (formal, e.g., "Art. 1 Bundesgesetz über den Datenschutz"), ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Swiss statute or provision is currently in force, amended, repealed, or not yet in force. ' +
    ...
    inputSchema: {
      ...
      properties: {
        document_id: {
          type: 'string',
          description: 'Statute identifier (SR number, abbreviation, or title).',
        },
        provision_ref: {
          type: 'string',
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that a Swiss statute aligns with or references. ' +
      'Switzerland is not an EU member but many Swiss laws align with EU directives/regulations ' +
      '(e.g., nFADP aligns with GDPR, ZertES with eIDAS). ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Swiss statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_swiss_implementations',
    description:
      'Find all Swiss statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Swiss statutes. ' +
      'Note: Switzerland implements EU law through bilateral agreements and autonomous alignment, not transposition.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Swiss implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_swiss_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Swiss aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Swiss statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Swiss statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_swiss_implementations':
          result = await getSwissImplementations(db, args as unknown as GetSwissImplementationsInput);
          break;
