/**
 * Golden contract tests for Switzerland Law MCP.
 *
 * Uses in-memory MCP client/server to call tools and verify outputs
 * against the golden-tests.json fixture file. Network-dependent tests
 * (upstream_text_hash, citation_resolves) are gated behind
 * CONTRACT_MODE=nightly via skipIf.
 */
...
describe(`Contract tests: ${fixture.mcp_name}`, () => {
  beforeAll(async () => {
    const dbPath =
      process.env['SWISS_LAW_DB_PATH'] ?? join(__dirname, '..', '..', 'data', 'database.db');
    db = new Database(dbPath, { readonly: true });

    const server = new Server(
      { name: 'switzerland-law-test', version: '0.0.0' },
      { capabilities: { tools: {} } },
    );
